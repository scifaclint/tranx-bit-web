...
export const RegisterForm = ({
    ...
    const handleUsernameBlur = useCallback(() => {
        const trimmedUsername = formData.username.trim();
        if (trimmedUsername.length >= 3 && usernameAvailable === null && !isCheckingUsername) {
            const checkAvailability = async () => {
                setIsCheckingUsername(true);
                try {
                    const response = await authApi.checkUsername(trimmedUsername);
    ...
    const isFormValid = useMemo(() => {
        ...
        const isValid =
            basicFieldsValid && allPasswordRequirementsMet && passwordNotForbidden && (usernameAvailable === true || usernameAvailable === null);
                                    ...
                                    <>
                                        ...
                                        {formData.username && (
                                            <button
                                                ...
                                                title="Clear field"
                                            >
                                                <X className={`h-4 w-4 ${usernameAvailable === false ? 'text-red-500' : 'text-muted-foreground'}`} />
                                            </button>
                                        )}
                                    </>
                                )}
