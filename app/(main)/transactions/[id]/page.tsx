...
export default function TransactionDetailsPage() {
  ...
  const getPaymentMethodImage = (method: any) => {
    ...
    for (const val of valuesToTry) {
      if (val && typeof val === "string") {
        ...
        // Priority check for paystack
        if (method.name === "Paystack") return PAYMENT_LOGOS["paystack"];
  ...
  const paymentImage = getPaymentMethodImage(transaction.paymentMethodSnapshot);
      ...
      <main className="container max-w-lg mx-auto p-4 sm:p-8 space-y-6">
        ...
        <div className="text-center space-y-2 py-6">
          ...
          <div className="space-y-1">
            ...
            <Badge
              variant="outline"
              className={`mt-2 border-zinc-200 dark:border-zinc-800 ${statusConfig.color} bg-background`}
            >
              {statusConfig.label}
            </Badge>
          </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <span className="text-sm font-medium capitalize">
                {transaction.type === "order_payment" ? "Third-party Gateway" : `${transaction.balanceSource} Wallet`}
              </span>
            </div>
          </CardContent>
        </Card>
              ...
              <div className="flex items-center gap-4 p-3 rounded-lg bg-zinc-50 dark:bg-zinc-950/50 border border-zinc-100 dark:border-zinc-800">
                ...
                <div className="space-y-1 overflow-hidden">
                  <p className="text-sm font-bold truncate">
                    {transaction.paymentMethodSnapshot?.name === "Paystack"
                      ? "Secure Checkout"
                      : (transaction.paymentMethodSnapshot?.accountName ||
                        transaction.paymentMethodSnapshot?.name ||
                        "Unknown Account")}
                  </p>
                  <p className="text-xs text-zinc-500 font-mono">
                    {transaction.paymentMethodSnapshot?.name === "Paystack"
                      ? "Automated Payment"
                      : (transaction.paymentMethodSnapshot?.mobileNumber ||
                        transaction.paymentMethodSnapshot?.walletAddress ||
                        transaction.paymentMethodSnapshot?.accountNumber)}
                  </p>
                </div>
                {(() => {
                  const netOrAsset =
                    transaction.paymentMethodSnapshot?.mobileNetwork ||
                    transaction.paymentMethodSnapshot?.cryptoAsset;
                  if (!netOrAsset || transaction.paymentMethodSnapshot?.name === "Paystack") return null;
                  return (
                    <div className="ml-auto text-xs font-medium px-2 py-1 rounded bg-zinc-200 dark:bg-zinc-800 capitalize">
                      {NETWORK_LABELS[netOrAsset.toLowerCase()] ||
                        netOrAsset.toUpperCase()}
                    </div>
                  );
                })()}
              </div>
